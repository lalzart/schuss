#!/usr/bin/env python3
"""Inspect or authenticate the uninstalled Tide Pit Release VST3 bundle."""
...
DEFAULT_BUILD = REPO_ROOT / "build/tide-pit-vst3-task047-release"
TASK047_CONTRACT = (
    REPO_ROOT / "research/prototypes/vst3-resampling/contract-r01"
)
RECEIPT = TASK047_CONTRACT / "tide-pit-vst3-build.json"
JUCE_MANIFEST = (
    REPO_ROOT
    / "research/prototype_support/instrument_lab/juce-8.0.15-source-tree.json"
)
SOURCE_LOCK = PROTOTYPE / "third_party/SOURCE_LOCK.json"
SOURCE_VALIDATOR = PROTOTYPE / "tests/validate_source_lock.py"
...
def source_receipt() -> dict[str, Any]:
    run([sys.executable, str(SOURCE_VALIDATOR)])
    document = json.loads(SOURCE_LOCK.read_text(encoding="utf-8"))
    tide_pit = document["tide_pit"]
    mutable = document["mutable_instruments"]
    return {
        "files": tide_pit["files"],
        "manifest_sha256": tide_pit["canonical_manifest_sha256"],
        "revision": tide_pit["revision"],
        "source_id": tide_pit["source_id"],
        "source_lock_sha256": sha256(SOURCE_LOCK),
        "mutable_package": {
            "package_id": mutable["package_id"],
            "package_revision": mutable["package_revision"],
            "source_package_manifest_sha256": mutable[
...
def input_paths() -> list[Path]:
    roots = [
        ...
        PROTOTYPE / "src",
        PROTOTYPE / "generated",
        PROTOTYPE / "third_party/SOURCE_LOCK.json",
        PROTOTYPE / "third_party/THIRD_PARTY_NOTICES.md",
        PROTOTYPE / "tools",
        REPO_ROOT / "research/proposals/tide-pit-vst3-local.md",
        ...
        REPO_ROOT / "packages/dsp_sources/mutable_ksoloti_v1/SOURCE_PACKAGE.json",
        JUCE_MANIFEST,
    ]
    result: list[Path] = []
    for root in roots:
...
def bundle_path(build: Path) -> Path:
    return build / "tide-pit-vst3_artefacts/Release/VST3/Tide Pit.vst3"
...
def document(build: Path) -> dict[str, Any]:
    ...
    expected_info = {
        "CFBundleDisplayName": "Tide Pit",
        "CFBundleExecutable": "Tide Pit",
        "CFBundleIdentifier": "dev.schuss.tide-pit.vst3.local",
    ...
    return {
        ...
        "parameter_contract": {
            "adapter_parameters": 16,
            "musical_parameters": 16,
            "parameter_fingerprint": "14672542899034963389",
            "schema": "schuss-tide-pit-vst3-state-v1",
            "wrapper_service_parameters": 2081,
        },
        "resampling_contract": {
        ...
        "schema_version": "tide-pit-fixed-rate-vst3-build-v1",
        "source_authority": source_receipt(),
        "status": "passed",
        "target": "tide-pit-vst3_VST3",
    }
...
def main() -> int:
    ...
    try:
        ...
        if arguments.inspect:
            ...
            print(
                "Tide Pit Release VST3 receipt passed; the bundle remains "
                "uninstalled and Ableton, endpoints, and listening were not opened"
            )
    except (OSError, RuntimeError, json.JSONDecodeError) as error:
        print(f"Tide Pit VST3 build validation failed: {error}", file=sys.stderr)
        return 1
    return 0
