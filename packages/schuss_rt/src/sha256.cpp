...
std::string digest(const std::uint8_t* input, std::size_t size) {
    std::vector<std::uint8_t> data;
    if (size != 0) data.assign(input, input + size);
    ...
    for (std::size_t offset = 0; offset < data.size(); offset += 64U) {
        ...
        for (std::size_t index = 0; index < words.size(); ++index) {
            ...
            const auto first = h + s1 + choice + k[index] + words[index];
...
std::string sha256_hex(std::string_view bytes) {
    return digest(reinterpret_cast<const std::uint8_t*>(bytes.data()), bytes.size());
}
