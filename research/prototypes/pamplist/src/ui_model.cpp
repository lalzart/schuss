...
void populateSequence(
    ...
    result.bottom = {{
        ...
        slot(SurfaceSemantic::sequence_phase, "PHASE",
            "Offsets where this lane falls within its own cycle. It changes alignment, not rate, hit count, or tone.",
            PresentationKind::phase, 0.0, 127.0, 1.0, controls.phase_u7),
        slot(SurfaceSemantic::sequence_shape, "SHAPE",
            "Forms the continuous Motion value sent to nonzero destinations. With only Trigger On and every other route Direct, Shape does not change the sound.",
        ...
        slot(SurfaceSemantic::sequence_rotation, "ROTATE",
            "Moves Hits around the 16-step pattern without changing their count. At Hits 0 or 16 every rotation is equivalent.",
...
void populateGlobal(
    ...
    result.guide = "COHERE brings all seven voices into one body. Root, Spread, Tail, Damp, Width, and Duck require Cohere above zero; Clear removes only that body's history.";
    result.top = {{
        slot(SurfaceSemantic::global_drive, "DRIVE",
            "Pressure into the shared body. Requires Cohere above zero and cannot leak into the exact dry mix.",
