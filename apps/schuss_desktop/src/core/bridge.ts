...
async function transport(request: DesktopOperationRequest, workspace: string | null): Promise<unknown> {
  const envelope = { request, workspace };
  if (isTauriRuntime()) {
    return invoke("dispatch_desktop_operation", { request, workspace });
  }
  if (import.meta.env.DEV) {
    const response = await fetch("/__schuss/operation", {
      method: "POST",
  ...
  throw new CoreOperationError(
    ...
    "The production renderer requires the Schuss desktop shell.",
  );
}
...
async function dispatch<T, O extends ReadOnlyRequest["operation"]>(
  ...
  try {
    const result = assertResult<T, O>(await transport(request, null), request.operation);
...
export async function dispatchDesktopOperation<T>(
  request: DesktopOperationRequest,
  workspace: string | null = null,
): Promise<T> {
  try {
    const raw = await transport(request, workspace);
    if (typeof raw !== "object" || raw === null || Array.isArray(raw)) {
      throw new CoreOperationError("BRIDGE_RESPONSE_INVALID", "Core result must be an object.");
    }
    const result = raw as OperationResult<T, string>;
    if (
      result.canonical_profile !== "schuss-canonical-json-v1" ||
