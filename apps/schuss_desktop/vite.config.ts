...
const BRIDGE_SCRIPT = resolve(APP_ROOT, "bridge/read_only_catalog_bridge.py");
...
function readOnlyDevelopmentBridge(): Plugin {
  return {
    name: "schuss-read-only-development-bridge",
    configureServer(server) {
      const bridge = new DevelopmentCoreBridge();
      server.httpServer?.once("close", () => bridge.close());
      server.middlewares.use("/__schuss/read-only-operation", async (request, response) => {
          ...
          response.end(
            JSON.stringify({
              error: {
                ...
                message: "The local read-only Schuss core bridge is unavailable.",
              },
...
export default defineConfig({
  clearScreen: false,
  plugins: [react(), readOnlyDevelopmentBridge()],
  server: {
    host: "127.0.0.1",
    port: 1420,
    strictPort: true,
