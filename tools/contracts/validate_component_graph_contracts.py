...
import validate_device_instrument_contracts as base
    ...
    for line_number, raw_line in enumerate(data.splitlines(), 1):
        ...
        try:
            record = json.loads(
                ...
                object_pairs_hook=base._object_pairs,
                parse_float=base._reject_float,
                parse_int=base._parse_int,
                parse_constant=lambda text: (_ for _ in ()).throw(
                    ValueError(f"non-finite JSON number {text!r} is prohibited")
                ),
            )
        except (UnicodeDecodeError, json.JSONDecodeError, ValueError) as exc:
            raise ValueError(f"{path}:{line_number}: invalid restricted JSON: {exc}") from exc
...
def _exact_key(record: dict[str, Any], id_field: str) -> tuple[str, int, str]:
    return record[id_field], record["revision"], record["content_hash"]


def _reference_key(reference: dict[str, Any], id_field: str) -> tuple[str, int, str]:
...
def _validate_structural_records(
    records: list[dict[str, Any]],
    schema: dict[str, Any],
    schema_name: str,
    schema_version: str,
    ...
    for error in base.validate_schema_annotations(schema):
        _diagnostic(diagnostics, "SCHEMA_CONTRACT_INVALID", schema_name, "$", error)

    valid: list[dict[str, Any]] = []
    for record in records:
        subject = _subject(record)
        base._scan_portability(record, subject, diagnostics)
        errors = base._schema_errors(record, schema, schema)
        for error in errors:
            _diagnostic(diagnostics, "SCHEMA_STRUCTURE_INVALID", subject, "$", error)
    ...
    for record in valid:
        ...
        expected = base.record_content_hash(record, schema)
...
def _canonical_member_hash(value: dict[str, Any]) -> str:
    data = base.canonical_json(value).encode("utf-8")
...
def validate_all_contracts(
    ...
    core = validate_component_graph_directory(contract_root, schema_root, repository_root)
    device_schema = base.load_json(schema_root / base.DEVICE_SCHEMA_NAME)
    instrument_schema = base.load_json(schema_root / base.INSTRUMENT_SCHEMA_NAME)
    ...
    device_instrument = base.validate_contract_values(
        devices,
        instruments,
        device_schema,
        instrument_schema,
    ...
    combined_diagnostics = sorted(
        core.summary["diagnostics"] + device_instrument["diagnostics"],
    ...
    return {
        ...
        "status": "invalid" if error_count else (
            "valid-with-historical-deferred" if deferred_count else "valid"
        ),
        "record_counts": {
            **core.summary["record_counts"],
            "device_profiles": len(devices),
            "instruments": len(instruments),
        },
        "reference_resolution": {
            **core.summary["reference_resolution"],
            "device_profiles": device_instrument["reference_resolution"]["device_profiles_resolved"],
        ...
        "evidence_levels": [
            ...
            {"level": "implementation-seam-map", "status": gate_status},
            {
                ...
                "status": "failed" if error_count else (
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument("contract_root", nargs="?", type=Path, default=repository_root / "contracts")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.canonical_record is not None:
        ...
        schema_name = {
            ...
            "device-profile": base.DEVICE_SCHEMA_NAME,
            "instrument": base.INSTRUMENT_SCHEMA_NAME,
        }[args.record_kind]
        try:
            schema = base.load_json(args.schema_root / schema_name)
            record = base.load_json(args.canonical_record)
            errors = base._schema_errors(record, schema, schema)
            if errors:
                raise ValueError("; ".join(errors))
            sys.stdout.buffer.write(base.canonical_record_bytes(record, schema) + b"\n")
            return 0
    ...
    try:
        ...
        summary = {
            ...
            "evidence_levels": [],
            "diagnostics": [
                base.Diagnostic("INPUT_READ_FAILED", "error", str(args.contract_root), "$", str(exc)).as_dict()
            ],
        }
    print(base.canonical_json(summary))
