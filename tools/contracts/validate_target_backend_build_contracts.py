...
import validate_component_graph_contracts as component
...
def _subject(record: dict[str, Any]) -> str:
    ...
    return component._subject(record)
...
def _validate_structural(
    ...
    for kind, values in records.items():
        ...
        candidates = component._validate_structural_records(
            values,
            schemas[kind],
            schema_name,
            schema_version,
...
def _schema_maximum_errors(
    ...
    schema = base._resolve_schema(schema, root_schema)
    if "oneOf" in schema:
        matches = [branch for branch in schema["oneOf"] if not base._schema_errors(value, branch, root_schema, location)]
        if len(matches) == 1:
...
def _validate_build_results(
    ...
    for result in values:
        ...
        if component._cycle_nodes(related_edges):
...
def validate_target_backend_build_directory(
    ...
    diagnostics: list[base.Diagnostic] = []
    upstream_summary = component.validate_all_contracts(contract_root, schema_root, repository_root)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.canonical_record is not None:
        if args.record_kind is None:
            ...
            return 2
        try:
            schema = base.load_json(args.schema_root / SCHEMA_SPECS[args.record_kind][0])
            ...
            return 1
    try:
        result = validate_target_backend_build_directory(args.contract_root, args.schema_root, repository_root)
        ...
        summary = {
            ...
            "evidence_levels": [],
            "resolution_traces": [],
            "diagnostics": [base.Diagnostic("INPUT_READ_FAILED", "error", str(args.contract_root), "$", str(exc)).as_dict()],
        }
