...
#error "Pamplist module tests require the JUCE VST3 host"
#endif
...
void setPhysical(
    ...
    parameterAt(processor, *index).setValue(
        pam::vst3NormalizePhysical(descriptor, physical));
}

void configureAudibleProgram(juce::AudioProcessor& processor) {
    setPhysical(processor, "pamp.lane1.rate", 8.0F);
    setPhysical(processor, "pamp.lane1.phase", 0.0F);
    setPhysical(processor, "pamp.lane1.shape", 1.0F);
    setPhysical(processor, "pamp.lane1.hits", 16.0F);
    setPhysical(processor, "pamp.lane1.chance", 1.0F);
    setPhysical(processor, "pamp.lane1.depth", 1.0F);
    setPhysical(processor, "pamp.lane1.motion.trigger", 1.0F);
    setPhysical(processor, "pamp.lane1.voice.model", 21.0F);
    setPhysical(processor, "pamp.lane1.voice.pitch", 36.0F);
    setPhysical(processor, "pamp.lane1.voice.level", 0.22F);
    setPhysical(processor, "pamp.master", 0.65F);
    setPhysical(processor, "pamp.run", 1.0F);
}
...
void run(const juce::File& bundle) {
    ...
    expect(description.name == "Pamplist"
    ...
    for (std::size_t index = 0; index < descriptors.size(); ++index) {
        ...
        expect(parameter.getName(256) == juce::String{descriptors[index].name},
    ...
    auto& bypass = parameterAt(*left, pam::kVst3ParameterCount);
    expect(&bypass == left->getBypassParameter()
            && bypass.getName(256) == "Bypass",
        "VST3 wrapper bypass parameter drift");
    for (std::size_t index = pam::kVst3ParameterCount + 1U;
    ...
    expect(editor != nullptr && editor->getWidth() >= 1080
            && editor->getHeight() >= 730 && !editor->isShowing(),
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        std::cerr << "pamplist_vst3_module_tests: " << error.what() << '\n';
        return 1;
    }
    std::cout << "pamplist_vst3_module_tests: PASS\n";
    return 0;
}
